...
static void
block_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
  Charmap *charmap;
  GtkTreeIter iter;
  GtkTreeModel *model;
...
make_unicode_block_selector (Charmap *charmap)
{
  ...
  g_signal_connect (G_OBJECT (selection), "changed", G_CALLBACK (block_selection_changed_cb), charmap);
...
static gchar *
character_name (gunichar uc)
{
  int min = 0;
  int mid;
  int max = sizeof (character_names) / sizeof (struct _charname);
  ...
  while (max >= min) 
    {
      mid = (min + max) / 2;
      if (uc > character_names[mid].index)
        min = mid + 1;
      else if (uc < character_names[mid].index)
        max = mid - 1;
      else
        return character_names[mid].name;
    }
...
static void
character_clicked_cb (GtkEventBox *event_box, GdkEvent *event, gpointer data)
{
  ...
  if (square != charmap->selected)
    {
      ...
      uc = charmap->block_start + (square->row * charmap->columns) + square->col;
...
static Square *
square_new (guint16 row, guint16 col, GtkWidget *event_box, GtkWidget *label, Charmap *charmap)
{
  Square *square;
      ...
      for (col = 0;  col < charmap->columns;  col++)
        {
          ...
          g_signal_connect (G_OBJECT (charmap->squares[row][col]->event_box), 
                            "button_press_event",
                            G_CALLBACK (character_clicked_cb), charmap->squares[row][col]);
...
charmap_init (Charmap *charmap)
{
  ...
  gtk_box_pack_start (GTK_BOX (hbox), charmap->character_selector, TRUE, TRUE, 0);
...
charmap_get_type ()
{
  ...
  if (!charmap_type)
	...
	sizeof (CharmapClass),
      ...
      charmap_type = g_type_register_static (GTK_TYPE_VBOX, "Charmap", &charmap_info, 0);
    }
...
{
  if (charmap->font_name == NULL || g_ascii_strcasecmp (charmap->font_name, font_name) != 0)
    {
      ...
      for (row = 0;  row < charmap->rows;  row++)
        for (col = 0;  col < charmap->columns;  col++)
          gtk_widget_modify_font (charmap->squares[row][col]->label, font_desc);
    }
}


/* the value returned should not be modified or freed */
gchar *
