...
{
  gucharmap_charmap_identify_clipboard (guw->charmap, 
  ...
  if (response == GTK_RESPONSE_OK)
    {
      ...
      if (*text != '\0' && *endptr == '\0' && l >= 0 && l <= UNICHAR_MAX)
        gucharmap_charmap_go_to_character (guw->charmap, (gunichar) l);
      else
        {
  ...
  if (visible)
    {
      ...
      gucharmap_charmap_show_caption (guw->charmap, caption_id);
    }
  else
    {
      guw->caption_show[caption_id] = FALSE;
      gucharmap_charmap_hide_caption (guw->charmap, caption_id);
    }
}
  ...
  if (gtk_check_menu_item_get_active (mi))
    {
      ...
      for (i = 0;  i < G_N_ELEMENTS (nameslist_caption_ids);  i++)
        if (guw->caption_show[nameslist_caption_ids[i]])
          gucharmap_charmap_show_caption (guw->charmap, nameslist_caption_ids[i]);
    }
  else
    {
      ...
      for (i = 0;  i < G_N_ELEMENTS (nameslist_caption_ids);  i++)
        gucharmap_charmap_hide_caption (guw->charmap, nameslist_caption_ids[i]);
    }
}
  ...
  if (gtk_check_menu_item_get_active (mi))
    {
      ...
      for (i = 0;  i < G_N_ELEMENTS (unihan_caption_ids);  i++)
        if (guw->caption_show[unihan_caption_ids[i]])
          gucharmap_charmap_show_caption (guw->charmap, unihan_caption_ids[i]);
    }
  else
    {
      ...
      for (i = 0;  i < G_N_ELEMENTS (unihan_caption_ids);  i++)
        gucharmap_charmap_hide_caption (guw->charmap, unihan_caption_ids[i]);
    }
}
#endif
  ...
  if (gtk_check_menu_item_get_active (mi))
    {
      ...
      for (i = 0;  i < G_N_ELEMENTS (unicode_caption_ids);  i++)
        if (guw->caption_show[unicode_caption_ids[i]])
          gucharmap_charmap_show_caption (guw->charmap, unicode_caption_ids[i]);
    }
  else
    {
      ...
      /* hide all the unicode captions */
      for (i = 0;  i < G_N_ELEMENTS (unicode_caption_ids);  i++)
        gucharmap_charmap_hide_caption (guw->charmap, unicode_caption_ids[i]);
    }
}
